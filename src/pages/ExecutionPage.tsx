...
import { executionCriteria } from '../data/mockData';
...
export const ExecutionPage: React.FC<ExecutionPageProps> = ({ accessLevel, userDepartment }) => {
  ...
  const subcategories: Array<'Показатели исполнения заказа (D)' | 'Производительность труда'> = [
  ...
  const groupedBySubcategory = subcategories.map(subcategory => ({
    subcategory,
    criteria: executionCriteria.filter(c => c.subcategory === subcategory),
  }));
